...
library(tidyverse)
library(keras)
data <- read_excel("D:/R/merged.xlsx", col_names = TRUE) 
aapl_data <- data %>% 
  filter(Symbol == "AAPL") %>%
  mutate(Date = as.Date(Date, format="%Y-%m-%d"))
read_and_transform <- function(sheet_name) {
  df <- read_excel("D:/R/Financial_Reports_20192023.xlsx", sheet = sheet_name)
  df <- df %>%
    mutate(Year = as.integer(format(as.Date(fiscalDateEnding, "%Y-%m-%d"), "%Y")))
  return(df)
}
FF <- read_csv("D:/R/F-F_Research_Data_Factors.csv", skip = 3, col_types = cols(...1 = col_integer(), `Mkt-RF` = col_double(),
